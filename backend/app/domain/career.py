...
from app.domain.curriculum import build_learning_unit, build_remediation_unit
...
TECH_RULES = [
    ...
    (
        "自动化测试",
        ["pytest", "junit", "playwright", "selenium", "自动化测试"],
        "qa.automation",
        "quality_engineering",
    ),
...
def parse_job_description(raw_text: str, source_url: str = "") -> dict[str, Any]:
    ...
    education = next(
        (value for value in ("博士", "硕士", "本科", "大专", "专科") if value in text), "未明确"
    )
    experience_match = re.search(
        r"(\d+\s*[-—~至]\s*\d+\s*年|\d+\s*年以上|应届生|在校生|不限经验)", text
    )
    salary_match = re.search(
        r"(\d+(?:\.\d+)?\s*[kKＫ]\s*[-—~至]\s*\d+(?:\.\d+)?\s*[kKＫ](?:\s*[·x×]\s*\d+薪)?|\d+\s*[-—~至]\s*\d+\s*万/年)",
        text,
    )
    ...
    responsibilities = [
        ...
        if any(
            word in line
            for word in ("负责", "参与", "设计", "开发", "维护", "建设", "优化", "实现")
        )
    ][:12]
    ...
    return {
        ...
        "suggested_tracks": [
            {"track_code": code, "evidence_count": count} for code, count in tracks[:4]
        ],
        "confidence": round(
            min(0.96, 0.38 + extracted_fields * 0.09 + min(0.22, len(skills) * 0.02)), 2
        ),
        "limitations": [
...
def job_gap_analysis(
    required_skills: list[dict[str, Any]], skill_scores: dict[str, float]
) -> dict[str, Any]:
    rows = []
        ...
        rows.append(
            {
                **item,
                ...
                "status": "已具备" if current >= target else "待补齐",
            }
        )
...
def build_revision(
    ...
    *,
    ...
    if trigger == "job_target" and target_skills:
        active = next(
            (phase for phase in new_phases if phase.get("status") != "completed"),
        ...
        if active:
                ...
                for phase in new_phases:
                    ...
                    for task in phase.get("tasks", []):
                        if task.get("skill_code") == code and existing_task is None:
                            ...
                            kept.append(task)
                    phase["tasks"] = kept
                topic = item.get("name", code)
                base_task = existing_task or build_learning_unit(
                    topic,
                    track_code="",
                    pathway_name="目标岗位差距路线",
                    stage_title="岗位能力前置",
                )
                prioritized.append(
                    {
                        **base_task,
                        ...
                        "title": f"岗位差距：{topic}",
                        "skill_code": code,
                        "priority": "job_required",
                        "job_alignment": f"完成后对照目标 JD 标注“{topic}”对应的职责原文",
                    }
                )
            ...
            changes.append(
                {
                    "type": "job_alignment",
                    "label": f"将 {len(prioritized)} 个岗位差距任务前置并标记为高优先级",
                }
            )

    if trigger in {"too_hard", "blocked"} and new_phases:
        active = next(
            (phase for phase in new_phases if phase.get("status") != "completed"), new_phases[0]
        )
        skill = next(iter(active.get("skills", [])), "core.programming")
        blocked_task = next(iter(active.get("tasks", [])), {})
        blocked_topic = blocked_task.get("topic") or blocked_task.get("title") or skill
        unit = build_remediation_unit(
            blocked_topic,
            track_code=blocked_task.get("track_code", ""),
        )
        support = {
            **unit,
            "id": f"support:{trigger}:{skill}",
            "skill_code": skill,
            "priority": "remediation",
            "scheduled_week": active.get("week_start", 1),
            "week_label": f"第 {active.get('week_start', 1)} 周",
        }
    ...
    if validated_skills:
        ...
        for phase in new_phases:
            ...
            phase["tasks"] = kept
        changes.append(
            {"type": "evidence_credit", "label": f"依据已验证成果免除 {removed} 个重复训练任务"}
        )
    ...
    if weekly_hours:
        original_hours = max(
            [int(phase.get("hours_per_week", weekly_hours) or weekly_hours) for phase in new_phases]
            or [weekly_hours]
        )
    ...
    for phase in new_phases:
        duration = max(
            1, int(phase.get("week_end", cursor)) - int(phase.get("week_start", cursor)) + 1
        )
        if phase.get("status") != "completed":
    ...
    if new_total != old_total:
        changes.append(
            {"type": "duration", "label": f"总周期由 {old_total} 周调整为 {new_total} 周"}
        )
    reason_map = {
    ...
    reason = reason_map.get(trigger, "学习状态变化触发路线校准") + (
