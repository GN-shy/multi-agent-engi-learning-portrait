    ...
    <div class="generate-layout">
      ...
      <section class="preview-column">
                  ...
                  <div class="phase-tasks">
                    <div v-for="task in phase.tasks.slice(0, 5)" :key="`${task.pathway_id}-${task.title}`">
                      <header class="unit-head">
                        <b>{{ task.title }}</b>
                        <i>{{ task.week_label }} · 约 {{ task.estimated_hours }} 小时</i>
                      </header>
                      <ol>
                        <li v-for="point in task.knowledge_points.slice(0, 4)" :key="point">{{ point }}</li>
                      </ol>
                      <span v-if="task.knowledge_points.length > 4">完整清单还有 {{ task.knowledge_points.length - 4 }} 项，进入学习计划后可逐项查看</span>
                      <small>{{ task.practice }}</small>
        ...
        <div class="panel pipeline-panel">
          <div class="panel-title"><div><h3>多智能体协作</h3><p>真实后端事件，不用前端动画冒充执行状态</p></div></div>
          <div class="pipeline">
...
async function generate() {
  ...
  try {
    ...
    ElMessage.success(result.value?.source_audit?.fallback_triggered ? '生成完成，部分外部能力已安全降级' : '多智能体协作路线已生成')
  } catch (error: any) {
