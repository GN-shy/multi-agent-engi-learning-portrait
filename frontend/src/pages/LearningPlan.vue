  ...
  <AppShell>
    ...
    <section v-if="plan" class="timeline">
      <article v-for="phase in plan.phases" :key="phase.id" class="phase panel" :class="phase.status">
        <header><span>{{ phase.week_start }}–{{ phase.week_end }} 周</span><el-tag :type="phase.status==='completed'?'success':phase.status==='active'?'primary':'info'">{{ phase.status }}</el-tag></header>
        <h3>{{ phase.name }}</h3><p>阶段里程碑：{{ phase.milestone }}</p>
        <el-progress :percentage="phase.progress || 0" />
        <div class="tasks">
          <el-checkbox v-for="skill in phase.skills" :key="skill" :model-value="checked(`${phase.id}:${skill}`)" @change="toggle(`${phase.id}:${skill}`,$event)">{{ skill }}</el-checkbox>
        </div>
    ...
    <DetailModal v-model="detail.visible" :title="detail.phase?.name || '阶段详情'"><el-descriptions :column="2" border><el-descriptions-item label="周期">第 {{ detail.phase?.week_start }}–{{ detail.phase?.week_end }} 周</el-descriptions-item><el-descriptions-item label="每周投入">{{ detail.phase?.hours_per_week }} 小时</el-descriptions-item><el-descriptions-item label="生成策略">{{ detail.phase?.strategy }}</el-descriptions-item><el-descriptions-item label="状态">{{ detail.phase?.status }}</el-descriptions-item></el-descriptions><h4>覆盖技能</h4><div class="tag-row"><el-tag v-for="skill in detail.phase?.skills||[]" :key="skill">{{ skill }}</el-tag></div><h4>验收里程碑</h4><p>{{ detail.phase?.milestone }}</p></DetailModal>
  </AppShell>
</template>

<script setup lang="ts">
import { onMounted,reactive,ref } from 'vue';import { useRouter } from 'vue-router';import { ElMessage } from 'element-plus'
import AppShell from '@/components/layout/AppShell.vue';import DetailModal from '@/components/common/DetailModal.vue';import { getData,postData } from '@/api'
...
<style scoped>
.plan-hero{display:flex;align-items:center;justify-content:space-between;background:linear-gradient(120deg,#fff,#edf3ff)}.plan-hero h2{font-size:28px;margin:12px 0 8px}.plan-hero p{color:var(--muted)}.timeline{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-top:18px}.phase{position:relative}.phase.active{border-color:#3168ee}.phase.completed{border-color:#8bd6bb}.phase header{display:flex;justify-content:space-between}.phase header>span{color:var(--muted);font-size:12px}.phase p{color:var(--muted);min-height:44px}.tasks{display:grid;gap:8px;margin:18px 0}.tasks .el-checkbox{margin:0}.checkin{margin-top:18px;display:flex;justify-content:space-between;align-items:center}.checkin h3,.checkin p{margin:5px}.checkin p{color:var(--muted)}@media(max-width:1100px){.timeline{grid-template-columns:repeat(2,1fr)}}@media(max-width:650px){.timeline{grid-template-columns:1fr}.plan-hero,.checkin{align-items:flex-start;flex-direction:column;gap:15px}}
</style>
