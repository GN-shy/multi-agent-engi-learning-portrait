  ...
  <AppShell>
    ...
    <section v-if="plan" class="timeline">
      <article v-for="phase in plan.phases" :key="phase.id" class="phase panel" :class="phase.status">
        ...
        <h3>{{ phase.name }}</h3><p v-if="phase.pathway_name" class="pathway-name">{{ phase.pathway_name }} · 原路线周期 {{ phase.source_duration }}</p><p>阶段里程碑：{{ phase.milestone }}</p>
