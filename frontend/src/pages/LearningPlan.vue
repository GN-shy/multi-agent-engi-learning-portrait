  ...
  <AppShell>
    <template v-if="plan">
      ...
      <section class="work-grid">
        <article class="panel weekly-board">
          <header class="section-head"><div><span class="eyebrow">第 {{ activeWeek }} 周执行</span><h3>{{ activePhase?.name || '等待下一阶段' }}</h3></div><el-tag type="success">{{ activeTasks.length }} 个任务</el-tag></header>
            ...
            <article v-for="task in activeTasks" :key="task.id" :class="['task-card',{done:checked(taskKey(activePhase,task))}]">
              ...
              <div>
                <div class="task-title">
                  <b>{{ task.title }}</b>
                  <el-tag size="small" effect="plain">{{ task.week_label || `第 ${activeWeek} 周` }} · 约 {{ task.estimated_hours || 1 }} 小时</el-tag>
                  <el-tag v-if="task.priority==='job_required'" type="danger" size="small">JD 高优先级</el-tag>
                </div>
                <ol class="knowledge-list"><li v-for="point in task.knowledge_points" :key="point">{{ point }}</li></ol>
                <p class="task-practice">{{ task.practice }}</p>
                <span>验收：{{ task.acceptance || activePhase?.milestone }}</span>
              </div>
    ...
    <DetailModal v-model="detail.visible" :title="detail.phase?.name || '阶段详情'">
      ...
      <div class="detail-tasks">
        <article v-for="task in phaseTasks(detail.phase||{})" :key="task.id">
          <header><b>{{ task.title }}</b><el-tag size="small" effect="plain">{{ task.week_label }} · 约 {{ task.estimated_hours }} 小时</el-tag></header>
          <h5>按顺序学习</h5>
          <ol class="knowledge-list"><li v-for="point in task.knowledge_points" :key="point">{{ point }}</li></ol>
          <p>{{ task.practice }}</p>
          <p>{{ task.failure_drill }}</p>
          <h5>自测清单</h5>
          <ul class="validation-list"><li v-for="item in task.validation" :key="item">{{ item }}</li></ul>
          <div class="search-terms"><strong>建议搜索：</strong><i v-for="term in task.search_terms" :key="term">{{ term }}</i></div>
          <span><strong>成果：</strong>{{ task.evidence_required }}</span>
          <span><strong>验收：</strong>{{ task.acceptance || detail.phase?.milestone }}</span>
        </article>
      </div>
    </DetailModal>
...
const activeWeek = computed(() => {
  if (!activePhase.value) return 1
  const firstIncomplete = phaseTasks(activePhase.value)
    .filter((task:any)=>!checked(taskKey(activePhase.value,task)))
    .sort((left:any,right:any)=>(left.scheduled_week||0)-(right.scheduled_week||0))[0]
  return firstIncomplete?.scheduled_week || activePhase.value.week_end || activePhase.value.week_start || 1
})
const activeTasks = computed(() => activePhase.value
  ? phaseTasks(activePhase.value).filter((task:any)=>(task.scheduled_week || activePhase.value.week_start)===activeWeek.value)
  : [])
